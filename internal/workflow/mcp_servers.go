...
func (s *Service) DeleteMCPServer(ctx context.Context, id string) error {
	...
	if _, err := s.execTxQuery(ctx, tx, `DELETE FROM agent_mcp_servers WHERE server_id = ?`, id); err != nil {
		return fmt.Errorf("delete agent_mcp_servers: %w", err)
	}
	res, err := s.execTxQuery(ctx, tx, `DELETE FROM mcp_servers WHERE id = ?`, id)
	if err != nil {
...
func (s *Service) SetAgentMCPServers(ctx context.Context, agentID string, serverIDs []string) error {
	...
	if _, err := s.execTxQuery(ctx, tx, `DELETE FROM agent_mcp_servers WHERE agent_id = ?`, agentID); err != nil {
		return fmt.Errorf("clear agent mcp servers: %w", err)
	}
	for _, sid := range serverIDs {
		if _, err := s.execTxQuery(ctx, tx, `INSERT INTO agent_mcp_servers (agent_id, server_id) VALUES (?, ?)`, agentID, sid); err != nil {
