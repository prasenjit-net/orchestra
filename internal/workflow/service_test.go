...
func TestDefinitionVersioningPublishesAndActivatesIndependently(t *testing.T) {
	...
	assertDefinitionVersionDrafts(t, service, ctx, definition.ID)
	assertDraftVersionRestrictions(t, service, ctx, definition.ID)
	assertPublishedInactiveVersionCanRun(t, service, ctx, definition.ID)
	assertDefinitionVersionActivation(t, service, ctx, definition.ID)
	assertPublishActivateAndReactivate(t, service, ctx, definition.ID)
	assertDefinitionVersionHistory(t, service, ctx, definition.ID)
}

func assertDefinitionVersionDrafts(t *testing.T, service *Service, ctx context.Context, definitionID string) {
	t.Helper()
	draftedV2, err := service.CreateDefinitionVersion(ctx, definitionID, CreateDefinitionInput{
		Name: "Versioned workflow", Description: "Tracks version lifecycle",
	...
	draftedV3, err := service.CreateDefinitionVersion(ctx, definitionID, CreateDefinitionInput{
		Name: "Versioned workflow from v1", Description: "Branches from the original version", BasedOnVersion: 1,
...
func assertDraftVersionRestrictions(t *testing.T, service *Service, ctx context.Context, definitionID string) {
	t.Helper()
	beforePublish, err := service.StartWorkflow(ctx, definitionID)
	if err != nil {
	...
	if _, err := service.StartWorkflowWithInput(ctx, StartWorkflowInput{
		DefinitionID: definitionID, DefinitionVersion: 2,
	}); !errors.Is(err, ErrVersionNotPublished) {
		t.Fatalf("expected draft version start to fail with ErrVersionNotPublished, got %v", err)
	}
	if _, err := service.ActivateDefinitionVersion(ctx, definitionID, 3); err == nil {
		t.Fatal("expected draft version activation to fail")
	}
}

func assertPublishedInactiveVersionCanRun(t *testing.T, service *Service, ctx context.Context, definitionID string) {
	t.Helper()
	published, err := service.PublishDefinitionVersion(ctx, definitionID, 2, false)
	if err != nil {
	...
	afterPublish, err := service.StartWorkflow(ctx, definitionID)
	if err != nil {
...
func assertDefinitionVersionActivation(t *testing.T, service *Service, ctx context.Context, definitionID string) {
	t.Helper()
	activatedV2, err := service.ActivateDefinitionVersion(ctx, definitionID, 2)
	if err != nil {
	...
	afterActivation, err := service.StartWorkflow(ctx, definitionID)
	if err != nil {
...
func assertPublishActivateAndReactivate(t *testing.T, service *Service, ctx context.Context, definitionID string) {
	t.Helper()
	publishedAndActivated, err := service.PublishDefinitionVersion(ctx, definitionID, 3, true)
	if err != nil {
	...
	reactivatedV1, err := service.ActivateDefinitionVersion(ctx, definitionID, 1)
	if err != nil {
...
func assertDefinitionVersionHistory(t *testing.T, service *Service, ctx context.Context, definitionID string) {
	t.Helper()
	reloaded, err := service.GetDefinition(ctx, definitionID)
	if err != nil {
